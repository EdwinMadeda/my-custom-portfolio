...
const Header = () => {
  ...
  const isResumeActive = pathname.toLowerCase().includes('resume');
  ...
  return (
    <header>
      <nav
        ...
        ${
          isNavSticky || isHamburgerActive || isResumeActive
      ...
      >
        <div className="flex justify-between items-center mx-auto px-5 md:px-10 lg:px-0 py-2 md:py-5 h-full w-full max-w-[1000px]">
          ...
          <NavLinks
            ...
            before:absolute before:h-[7px] before:w-[7px] before:rounded-full 
            before:bg-[var(--color-primary2)] before:dark:bg-white
            before:left-1/2 before:-translate-x-1/2 before:-translate-y-1/2 before:-bottom-3 

            hover:before:visible  
          ${isActive ? 'before:visible' : 'before:invisible'}
          `}
            linkTextClass={() =>
              'block uppercase text-[0.8em] md:text-[0.87em] transition-all duration-400 group-hover:invisible dark:text-white'
            }
          />
          <button
